...
import { workoutDefinition } from '../src/training/engine';
...
export default function History() {
  ...
  return <Screen title="Your progress." eyebrow="ONE SESSION AT A TIME">
    {!ready ? <Message>{error || 'Loading training…'}</Message> : !data.profile ? <Button title="Set up my training" onPress={() => router.push('/onboarding')} /> : <>
      ...
      {data.history.map(entry => <View key={entry.id} style={s.card}><Text style={s.heading}>Workout {entry.training.nextWorkout}</Text><Text style={s.muted}>{new Date(entry.completedAt).toLocaleString()}</Text>{workoutDefinition(entry.training.nextWorkout).map(({ lift }) => <View key={lift} style={{ gap: 4 }}><Text style={s.text}>{liftNames[lift]} · {entry.training.lifts[lift].weight} {entry.training.unit}</Text><Text style={s.muted}>{entry.reps[lift]?.join(' / ')} reps</Text></View>)}</View>)}
    </>}
  </Screen>;
}
