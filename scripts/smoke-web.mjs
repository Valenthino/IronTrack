...
try {
  ...
  await waitFor(contains('2 / 5'));
  await evaluate("[...document.querySelectorAll('[role=radio]')].find(el => el.textContent.includes('Feel confident')).click()");
  await click('Continue →');
  await waitFor(contains('3 / 5'));
  await evaluate("[...document.querySelectorAll('[role=radio]')].find(el => el.textContent.startsWith('kg')).click()");
  await click('Continue →');
  await waitFor(contains('4 / 5'));
  assert.ok(await evaluate(contains('empty 20 kg bar')));
  await click('Continue →');
